...
<?php        
    ...
    //$teamRows = $wpdb->get_results(" SELECT * FROM {$wpdb->prefix}jgteams WHERE `pageurl`<>'';"); 
    //print_R($_SESSIOn);
    if(isset($_REQUEST['shrt']) && isset($_POST['jg_team_id']) && 
        ...
        */
        ...
        if ($myteam != null && !$client->Team->Exists($_POST['jg_team_shortname']) && isset($_POST['jg_page_id'])) {
            //update db and create page
        ...
        elseif(!$client->Team->Exists($_POST['jg_team_shortname']) && isset($_POST['jg_page_id'])){
            //insert
            $request = array();
            $request['teamShortName'] = $_POST['jg_team_shortname'] ;
            $request['name'] = $_POST['jg_team_name'];
        ...
        else
        {
            ...
            $defpage = stripslashes($_POST['jg_page_id']);
            $defteam = stripslashes($_POST['jg_team_shortname']);
            $teamname = stripslashes($_POST['jg_team_name']);
            $teamstory = stripslashes($_POST['jg_teamstory']);
            $deftype = stripslashes($_POST['jg_teamtype']);
            $deffbpage = stripslashes($_POST['jg_teamfbpage']);
            $deftwpage = stripslashes($_POST['jg_teamtwpage']);                               
            $deftargettype =  stripslashes($_POST['jg_teamtargettype']) ; 
            $teamtarget = stripslashes($_POST['jg_team_target']);
        }
    }
    elseif(!isset($_REQUEST['shrt']) && isset($_REQUEST['jg_page_id']) && 'POST' == $_SERVER['REQUEST_METHOD'])
    {
 
        if(isset($_POST['jg_team_shortname']) && !$client->Team->Exists($_POST['jg_team_shortname'])){
            //insert
    ...
    if (isset($_REQUEST['shrt'])){
        foreach ($teamRows as $team){
            if ($team->teamshortname == $_REQUEST['shrt'] && trim($_REQUEST['shrt']) !== '' ) {
                $fndit = true;
                $defteam = stripslashes($team->teamshortname);
                $teamname = stripslashes($team->teamname);
                $teamstory = stripslashes($team->teamstory);
                $deftype = stripslashes($team->teamtype);
                $deffbpage = stripslashes($team->teamfbpage);
                $deftwpage = stripslashes($team->teamtwpage);                   
                $deftargettype = stripslashes($team->teamtargettype) ;
                $teamtarget = stripslashes($team->teamtarget);
            }
        }
    }
    ...
    elseif(isset($_REQUEST['shrt']) && trim($_REQUEST['shrt']) != '' && !$client->Team->Exists($_REQUEST['shrt'])){
        $pages = $client->Event->RetrievePages($wpjg_generalSettings['Event']);
        foreach ($pages->fundraisingPages as $page)
        {
            $cpages[] = $page;
...
<?php
if (!$fndit || !$client->Team->Exists($_POST['jg_team_shortname']))
{
?>
