 ...
 * Project Home Page: http://uplFlickr.com/
 * Released under GNU Lesser General Public License (http://www.gnu.org/copyleft/lgpl.html)
 * For more information about the class and upcoming tools and toys using it,
 * visit http://www.uplFlickr.com/
 *
 ...
 *	 http://www.uplFlickr.com/README.txt
 *
 *	 Please submit all problems or questions to the Help Forum on my Google Code project page:
 *		 http://code.google.com/p/uplFlickr/issues/list
 *
 */ 
if ( !class_exists('uplFlickr') ) {
	if (session_id() == "") {
		@session_start();
	}
	...
	class uplFlickr {
		...
		function uplFlickr ($api_key, $secret = NULL, $die_on_error = false) {
		...
		function request ($command, $args = array(), $nocache = false)
		{
			...
			} elseif (!empty($_SESSION['uplFlickr_auth_token'])) {
				$args = array_merge($args, array("auth_token" => $_SESSION['uplFlickr_auth_token']));
			}
			ksort($args);
			$auth_sig = "";
		...
		function setProxy ($server, $port) {
			// Sets the proxy for all uplFlickr calls.
		...
		function getFriendlyGeodata ($lat, $lon) {
			 ...
			 */
			return unserialize(file_get_contents('http://uplFlickr.com/geodata/?format=php&lat=' . $lat . '&lon=' . $lon));
		}

		function sync_upload ($photo, $title = null, $description = null, $tags = null, $is_public = null, $is_friend = null, $is_family = null, $hidden = null) {
			if ( function_exists('curl_init') ) {
				// Has curl. Use it!

				//Process arguments, including method and login data.
				$args = array("api_key" => $this->api_key, "title" => $title, "description" => $description, "tags" => $tags, "is_public" => $is_public, "is_friend" => $is_friend, "is_family" => $is_family, "hidden" => $hidden);
				if (!empty($this->token)) {
					$args = array_merge($args, array("auth_token" => $this->token));
				} elseif (!empty($_SESSION['uplFlickr_auth_token'])) {
					$args = array_merge($args, array("auth_token" => $_SESSION['uplFlickr_auth_token']));
		...
		function async_upload ($photo, $title = null, $description = null, $tags = null, $is_public = null, $is_friend = null, $is_family = null, $hidden = null) {
			if ( function_exists('curl_init') ) {
				// Has curl. Use it!

				//Process arguments, including method and login data.
				$args = array("async" => 1, "api_key" => $this->api_key, "title" => $title, "description" => $description, "tags" => $tags, "is_public" => $is_public, "is_friend" => $is_friend, "is_family" => $is_family, "hidden" => $hidden);
				if (!empty($this->token)) {
					$args = array_merge($args, array("auth_token" => $this->token));
				} elseif (!empty($_SESSION['uplFlickr_auth_token'])) {
					$args = array_merge($args, array("auth_token" => $_SESSION['uplFlickr_auth_token']));
		...
		function auth ($perms = "read", $remember_uri = true) {
			...
			if (empty($_SESSION['uplFlickr_auth_token']) && empty($this->token)) {
				if ( $remember_uri === true ) {
					$_SESSION['uplFlickr_auth_redirect'] = $_SERVER['REQUEST_URI'];
				} elseif ( $remember_uri !== false ) {
					$_SESSION['uplFlickr_auth_redirect'] = $remember_uri;
				}
		...
		To use the uplFlickr::call method, pass a string containing the API method you want
		...
		*/
		...
		function auth_getToken ($frob) {
			...
			$_SESSION['uplFlickr_auth_token'] = $this->parsed_response['auth']['token'];
			return $this->parsed_response ? $this->parsed_response['auth'] : false;
		}
...
if ( !class_exists('uplFlickr_pager') ) {
	class uplFlickr_pager {
		var $uplFlickr, $per_page, $method, $args, $results, $global_uplFlickr;
		var $total = null, $page = 0, $pages = null, $photos, $_extra = null;
		
		
		function uplFlickr_pager($uplFlickr, $method = null, $args = null, $per_page = 30) {
			$this->per_page = $per_page;
			$this->method = $method;
			$this->args = $args;
			$this->set_uplFlickr($uplFlickr);
		}
		
		function set_uplFlickr($uplFlickr) {
			if ( is_a($uplFlickr, 'uplFlickr') ) {
				$this->uplFlickr = $uplFlickr;
				if ( $this->uplFlickr->cache ) {
		...
		function load($page) {
			...
			if ( $this->uplFlickr->cache ) {
				...
				if ( floor($min/500) == floor($max/500) ) {
					...
					$this->results = $this->uplFlickr->call($this->method, $this->args);
					if ( $this->results ) {
