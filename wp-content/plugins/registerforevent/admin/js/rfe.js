var data = {
    "rfeADDclose": parent.tinyMCE.util.I18n.translate('Insert and Close'),
    "rfeADD": parent.tinyMCE.util.I18n.translate('Insert'),
    "pages": []
};
function rfeinit() {
    $(function() {
        $('#rfe-type').change(function() {
            //console.log('#jg'+$('#justgiving-type option:selected').val());
            $('#rfe'+$('#rfe-type option:selected').val()).show();   
            $('#rfetype').hide();            
            $('.choice.submit').show();            
        });
...
function I_Insert() {
    //$('#jgtype').show();
    var shortcode = '[rfe-' + $('#rfe-type option:selected').val();
    $('#rfe'+$('#rfe-type option:selected').val()+' :input').each(function(){
        var valv = $(this).val();
        var showm = false;
...
$.get('forma.html', function(template) {
    $.post('../../../../../wp-admin/admin-ajax.php', 'action=rfe_listpages',function(pages){
