...
function afg_admin_menu() {
    add_menu_page('Flickr Gallery', 'Flickr Gallery', 'create_users', 'afg_plugin_page', 'afg_admin_html_page', BASE_URL . "/images/afg_logo.png", 898);
    $afg_main_page = add_submenu_page('afg_plugin_page', 'Default Settings | Flickr Gallery', 'Default Settings', 'create_users', 'afg_plugin_page', 'afg_admin_html_page');
    $afg_add_page = add_submenu_page('afg_plugin_page', 'Add Gallery | Flickr Gallery', 'Add Gallery', 'moderate_comments', 'afg_add_gallery_page', 'afg_add_gallery');
    $afg_saved_page = add_submenu_page('afg_plugin_page', 'Saved Galleries | Flickr Gallery', 'Saved Galleries', 'moderate_comments', 'afg_view_edit_galleries_page', 'afg_view_delete_galleries');
    /*$afg_sports_page = add_submenu_page('afg_plugin_page', 'Sports | Flickr Gallery', 'Sports', 'moderate_comments', 'afg_view_edit_sports_page', 'afg_view_edit_sports');*/
    $afg_edit_page = add_submenu_page('afg_plugin_page', 'Edit Galleries | Flickr Gallery', 'Edit Galleries', 'moderate_comments', 'afg_edit_galleries_page', 'afg_edit_galleries');
    $afg_advanced_page = add_submenu_page('afg_plugin_page', 'Advanced Settings | Flickr Gallery', 'Advanced Settings', 'create_users', 'afg_advanced_page', 'afg_advanced_settings_page');
...
function afg_admin_init() {
    ...
    register_setting('afg_settings_group', 'afg_users_photoset');  
    register_setting('afg_settings_group', 'afg_banned_photoset');
    register_setting('afg_settings_group', 'afg_report_photoset');  
    register_setting('afg_settings_group', 'afg_kids_photoset');  
    register_setting('afg_settings_group', 'afg_celebs_photoset');  
    register_setting('afg_settings_group', 'afg_flattwins_photoset');  
...
function afg_get_all_options() {
    return array(
        ...
        'afg_users_photoset' => get_option('afg_users_photoset'),  
        'afg_banned_photoset' => get_option('afg_banned_photoset'),
        'afg_report_photoset' => get_option('afg_report_photoset'),  
        'afg_kids_photoset' => get_option('afg_kids_photoset'),  
        'afg_celebs_photoset' => get_option('afg_celebs_photoset'),  
        'afg_flattwins_photoset' => get_option('afg_flattwins_photoset'),               
...
function afg_auth_init() {
    ...
    $pf->setToken('');
    $pf->auth('write', $_SERVER['HTTP_REFERER']);
    exit;
}
...
upgrade_handler();
    ...
    if ($_POST) {
        ...
        if (isset($_POST['submit']) && $_POST['submit'] == 'Delete Cached Galleries') {
            ...
            update_option('afg_kids_photoset', $_POST['afg_kids_photoset']);  
            update_option('afg_celebs_photoset', $_POST['afg_celebs_photoset']);  
            update_option('afg_flattwins_photoset', $_POST['afg_flattwins_photoset']);            
            update_option('afg_banned_photoset', $_POST['afg_banned_photoset']);
            update_option('afg_report_photoset', $_POST['afg_report_photoset']);
            update_option('afg_users_photoset', $_POST['afg_users_photoset']);
...
<?php
$photosets_map = array();
$flickrid = get_option('afg_user_id');
if (trim($flickrid) != ''){
    $rsp_obj = $pf->photosets_getList($flickrid);
    if (!$pf->error_code) {
        foreach($rsp_obj['photoset'] as $photoset) {
            $photosets_map[$photoset['id']] = $photoset['title']['_content'];
        }
    }
    $bann_photosets = afg_generate_options($photosets_map, get_option('afg_banned_photoset'), True);
    $rep_photosets = afg_generate_options($photosets_map, get_option('afg_report_photoset'), True);
    $users_photosets = afg_generate_options($photosets_map, get_option('afg_users_photoset'), True);

    $kids_photoset = afg_generate_options($photosets_map, get_option('afg_kids_photoset'), True);
    $celebs_photoset = afg_generate_options($photosets_map, get_option('afg_celebs_photoset'), True);  
    $flattwins_photoset = afg_generate_options($photosets_map, get_option('afg_flattwins_photoset'), True);             
?>
        <tr valign="top">
            <th scope="row">photoset for Banned images</th>
            <td><select name="afg_banned_photoset" id="afg_banned_photoset"><option value=""></option><?php echo $bann_photosets;?></select></td>
        </tr>
        <tr valign="top">
            <th scope="row">photoset for Reported images</th>
            <td><select name="afg_report_photoset" id="afg_report_photoset"><option value=""></option><?php echo $rep_photosets;?></select></td>
        </tr>
        <tr valign="top">
            <th scope="row">photoset for Users images</th>
            <td><select name="afg_users_photoset" id="afg_users_photoset"><option value=""></option><?php echo $users_photosets;?></select></td>
        </tr>  
        <tr valign="top">
            <th scope="row">photoset for sport kids images</th>
            <td><select name="afg_kids_photoset" id="afg_kids_photoset"><option value=""></option><?php echo $kids_photoset;?></select></td>
        </tr>   
        <tr valign="top">
            <th scope="row">photoset for twinned and user images</th>
            <td><select name="afg_flattwins_photoset" id="afg_flattwins_photoset"><option value=""></option><?php echo $flattwins_photoset;?></select></td>
        </tr> 
        <tr valign="top">
            <th scope="row">photoset for celeb images</th>
            <td><select name="afg_celebs_photoset" id="afg_celebs_photoset"><option value=""></option><?php echo $celebs_photoset;?></select></td>
        </tr>          
<?php        
}
?>
