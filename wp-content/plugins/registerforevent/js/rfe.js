...
(function($){
    ...
    var scriptPath = function () {
        ...
        if(scripts && scripts.length>0) {
            for(var i in scripts) {
                if(scripts[i].src && scripts[i].src.match(/\/rfe\.js.*$/)) {                    
                    path = scripts[i].src.replace(/(.*)\/rfe\.js.*$/, '$1');
                    //alert(path);
                    break;
                }
            }
    ...
    $('.js-ticket-container').on('click', '.genericon-cog', function() {
        $( this ).parents('.js-ticket-row').find('.ticket-settings').slideDown(); 
    });     
    $('.js-ticket-container').on('click', '.genericon-trash', function() {
        if ($( this ).parents('.js-ticket-container').find('.js-ticket-row').length > 1) $( this ).parents('.js-ticket-row').remove();
    });     
    $('.js-ticket-container').on('click', '.js-minimize-settings-link', function() {
        $( this ).parents('.ticket-settings').slideUp(); 
    }); 
    $('#tabs-2').on('click', '#id_group-tickets-1-another_ticket', function() {
        $( this ).parents('#tabs-2').find('.js-ticket-row').clone().appendTo( ".js-ticket-container" );
    });     
    ...
    */
    ...
    $(window).load(function(){
        $(document).ready(function(){  
            ...
            $("#event-adress")
              .geocomplete({details: "fieldset.location",types: ["geocode", "establishment"],detailsAttribute: "data-geo"})
              .bind("geocode:result", function(event, result){
                $('#adress').val(result.formatted_address.match(/^(.+?),/)[1]);
            });            
            if ($("#adduser").length > 0 || $("#login_form").length > 0 || 
                ...
                if (typeof($.validator) == 'undefined' ) {   
                    ...
                    if (pathy.length == 0) pathy = 'wp-content/plugins/eventbritedg/js';
