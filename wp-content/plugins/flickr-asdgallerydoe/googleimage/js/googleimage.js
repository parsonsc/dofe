...
*/        
	...
	function initEvents() {
		...
		// on window resize get the window size again
		// reset some values..
		...
		update : function( $item ) {
            ...
            $jq( '<img/>' ).load( function() {
                ...
                if( $img.attr( 'src' ) === self.$item.find('a').data( 'largesrc' ) ) {
                    ...
                    furl = updateQueryStringParameter(furl, 'id',  self.$item.find('a').data( 'did' ) );
                    if (getParameterByName('showtwin') == 1 && getParameterByName('id') == self.$item.find('a').data( 'did' ))
                    {
                        furl = updateQueryStringParameter(furl, 'showtwin',  1 );
                        self.$socialmedia.append('<a data-provider="facebook" target="_blank" rel="nofollow" title="Share on Facebook" href="https://www.facebook.com/dialog/feed?app_id=801984773168530&display=page&caption='+encodeURIComponent($jq('meta[property=og\\:title]').attr("content"))+'&link='+encodeURIComponent(furl)+'&redirect_uri='+furl+'&picture='+ self.$item.find('a').data( 'flat' )  +'&description='+$jq('meta[property=og\\:description]').attr("content")+'"><img src="wp-content/themes/twentythirteen-child/images/facebook-icon.png" alt="">Share this</a>');
                    }
                    else
                    {
                        url = updateQueryStringParameter(url, 'showtwin',  '' );
                        self.$socialmedia.append('<a data-provider="facebook" target="_blank" rel="nofollow" title="Share on Facebook" href="https://www.facebook.com/dialog/feed?app_id=801984773168530&display=page&caption='+encodeURIComponent('UNICEF - SportFirst')+'&link='+encodeURIComponent(url)+'&redirect_uri='+url+'&picture='+ self.$item.find('a').data( 'largesrc' )  +'&description=Star on a big screen at the Commonwealth Games. Share your Sport First moment with UNICEF and appear on a big screen in Glasgow. On your marks, get set, upload!"><img src="wp-content/themes/twentythirteen-child/images/facebook-icon.png" alt="">Share this</a>');
                    }
