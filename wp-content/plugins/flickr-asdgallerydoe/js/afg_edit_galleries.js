...
function getPhotoSourceType() {
    ...
    if (source_element.value == 'photostream' || source_element.value == 'popular') {
        ...
        if (photosets_box.length == 0) {
            ...
            tags_box.style.display = 'none';
            help_text.style.display = 'none';
            source_label.innerHTML = "Select Photosets";
        }
    }
    else if (source_element.value == 'group') {
        if (groups_box.length == 0) {
    ...
    else if (source_element.value == 'users') {
        source_label.style.display = 'none';
        photosets_box.style.display = 'none';
        galleries_box.style.display = 'none';
        groups_box.style.display = 'none';
        tags_box.style.display = 'none';
        help_text.style.display = 'none';
        source_label.innerHTML = "Users photos";
    } 
    else if (source_element.value == 'celebs') {
        source_label.style.display = 'none';
        photosets_box.style.display = 'none';
        galleries_box.style.display = 'none';
        groups_box.style.display = 'none';
        tags_box.style.display = 'none';
        help_text.style.display = 'none';
        source_label.innerHTML = "Celebs photos";
    }     
    else if (source_element.value == 'userscelebs') {
        source_label.style.display = 'none';
        photosets_box.style.display = 'none';
        galleries_box.style.display = 'none';
        groups_box.style.display = 'none';
        tags_box.style.display = 'none';
        help_text.style.display = 'none';
        source_label.innerHTML = "Users &amp; Celebs photos";
    }     
}
...
function loadGallerySettings() {
    ...
    if (source_element.value == 'photoset') {
        if(active_gallery.photoset_id.indexOf(',') === -1){
            photosets_box.value = active_gallery.photoset_id;
        }
        else{
            var sets = active_gallery.photoset_id.split(",");
            for ( var i = 0, l = photosets_box.options.length, o; i < l; i++ )
            {
              o = photosets_box.options[i];
              if ( sets.indexOf( o.value ) != -1 )
              {
                o.selected = true;
              }
            }
        ...
        if(active_gallery.gallery_id.indexOf(',') === -1){
            galleries_box.value = active_gallery.gallery_id;
        }
        else{
            var sets = active_gallery.gallery_id.split(",");
            for ( var i = 0, l = galleries_box.options.length, o; i < l; i++ )
            {
              o = galleries_box.options[i];
              if ( sets.indexOf( o.value ) != -1 )
              {
                o.selected = true;
              }
        ...
        if(active_gallery.group_id.indexOf(',') === -1){
            groups_box.value = active_gallery.group_id;
        }
        else{
            var sets = active_gallery.group_id.split(",");
            for ( var i = 0, l = groups_box.options.length, o; i < l; i++ )
            {
              o = groups_box.options[i];
              if ( sets.indexOf( o.value ) != -1 )
              {
                o.selected = true;
              }
