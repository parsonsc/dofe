    ...
    if (img.length > 0){
        ...
        if(img.indexOf("uperror") != -1 || img.indexOf("uploads/big") == -1){
            ...
            if (img.indexOf("uperror") != -1){
                jQuery('.noimg-loaded p.error', window.parent.document).html( jQuery(img).find('p').text() );
    ...
    function setupValidation (){
		...
		$("#ajaxafgflickr").validate({
			...
			messages: {
				...
				afgconsent: "Please agree to allow this image to be used on the website",                
            ...
            submitHandler: function(form){
                ...
                if ( $.isFunction( ga ) ) {
                    ga('send','event', 'Upload', 'Upload_Complete', 'Upload_Complete_Button', 1);
                }
                $('.submitGif').show();
                $('.submitGif').offset({
    ...
    $("#yourfile").on('change',function(){
		...
		if($("#yourfile").val()!=''){
            ...
            if (!blnValid){
                ...
                var loadingGif = 'http://sportfirst.unicef.org.uk/wp-content/themes/twentythirteen-child/images/UNICEF_loading.gif';
